...
# 1. 讀取經過分詞處理的 XLSX 文件
file_path = 'Segmented_Cleaned_延伸我.xlsx'  # 替換為你的 XLSX 檔案路徑
...
# 4. 將每個課程的詞嵌入向量計算出來，並轉換為逗號分隔、並用[]括起來的字串
...
output_file = 'Vectorized_延伸我.xlsx'
df[['課程名稱', 'vector']].to_excel(output_file, index=False)  # 使用 to_excel 保存為 Excel
